...
const getContext = (): Context =>
  createMetaplex().use({
    install(metaplex) {
      ...
      metaplex.use(nftStorageUploader());
    },
  });
...
// test.skip('[nftStorage] it can upload multiple files in batch', async (t) => {
...
// test.skip('[nftStorage] it can keep track of upload progress', async (t) => {
//   // Given a Metaplex instance using NFT.Storage.
//   const mx = await metaplex();
//   mx.use(nftStorage());
